...
export const ScrambleWords = () => {
    ...
    const handleGuessSubmit = (e: React.FormEvent) => {
        ...
        if (guess === currentWord) {
            ...
            if (newWords.length > 0) {
                const otherWord = newWords[0];
                const newScrambleWord = scrambleWord(otherWord);
                setWords(newWords);
                setCurrentWord(otherWord);
                setScrambledWord(newScrambleWord);
                setGuess('');
                setPoints(points + 1);
                return;
            }
    ...
    const handleSkip = () => {
        ...
        const otherWords = shuffleArray(words);
        const newWordScramble = otherWords[0];
        setCurrentWord(newWordScramble);
        setScrambledWord(scrambleWord(newWordScramble));
        setGuess('');
    };

    const handlePlayAgain = () => {
        const newListWords = shuffleArray(GAME_WORDS);
        const newWord = newListWords[0];
